...
def average_color_fusing(
    ...
    for i in range(n_chunk):
        ...
        appearance_embeddings = renderer.model.appearance_embedding(appearance_ids).reshape((
        ...
        if renderer.model.config.is_view_dependent:
        ...
        rgb_offset_for_each_camera[:, left:right, :] = renderer.model.appearance_network(input_features).reshape((gaussian_model.n_gaussians, real_chunk_size, -1)).to(device=device)
...
def average_embedding_fusing(
    ...
    embedding_network = renderer.model.appearance_network
    input_tensor_list = [
