...
@dataclass
class ColmapDino(DataParserConfig):
    """
        Args:
    ...
    def instantiate(self, path: str, output_path: str, global_rank: int) -> DataParser:
        return ColmapDinoDataParser(path, output_path, global_rank, self)


class ColmapDinoDataParser(DataParser):
    def __init__(self, path: str, output_path: str, global_rank: int, params: ColmapDino) -> None:
        super().__init__()
        self.path = path
        ...
        # Added by Yash
        self.dino_embeddings_path = os.path.join(self.path, "dino_embeddings")
        self.dino_details_path = os.path.join(self.path, "dino_details.json")
        
        # Assert that the required folder and file exist
        if not os.path.isdir(self.dino_embeddings_path):
            raise FileNotFoundError(f"Expected dino embeddings folder not found at: {self.dino_embeddings_path}")
        if not os.path.isfile(self.dino_details_path):
            raise FileNotFoundError(f"Expected dino details file not found at: {self.dino_details_path}")
        
        with open(self.dino_details_path, 'r') as f:
            self.dino_details = json.load(f)
        # Added by Yash
    ...
    def rotation_matrix(a, b):
        ...
        if c < -1 + 1e-8:
            ...
            return ColmapDinoDataParser.rotation_matrix(a + eps, b)
    ...
    def get_outputs(self) -> DataParserOutputs:
        ...
        # Load DiNO details to get the embedding size
        with open(self.dino_details_path, "r") as f:
            self.dino_details = json.load(f)  # Store dino_details as class variable
        dino_embedding_size = self.dino_details.get("embedding_dim")
        ...
        # Load DiNO embeddings per image
        dino_embeddings_per_image = {}
        for image in images.values():  # Use images.values() to iterate over image objects
            image_name = image.name  # e.g., "0001.jpg"
            # embedding_file = os.path.join(self.dino_embeddings_path, f"{image_name}.npy")
            image_basename = os.path.splitext(image_name)[0]  # removes '.jpg'
            embedding_file = os.path.join(self.dino_embeddings_path, f"{image_basename}.npy")

            if os.path.exists(embedding_file):
                emb = np.load(embedding_file)
                emb = torch.tensor(emb, dtype=torch.float)
                # Optionally, validate the embedding dimension
                # expected_dim = self.dino_details.get("embedding_dim", 384) # Use class variable
                expected_dim = dino_embedding_size
                if emb.shape[0] != expected_dim:
                    raise ValueError(
                        f"Embedding for {image_name} has dimension {emb.shape[0]}, expected {expected_dim}"
                    )
                dino_embeddings_per_image[image.name] = emb
            else:
                raise FileNotFoundError(f"Embedding file for image {image_name} not found in {self.dino_embeddings_path}")
        ...
        # Calculate average DiNO embeddings per appearance ID
        dino_embeddings_per_appearance = {}
        for appearance_group_name, appearance_id_tuple in appearance_group_name_to_appearance_id.items():
            appearance_id = appearance_id_tuple[0]  # Get the integer ID
            group_image_names = appearance_groups[appearance_group_name]
            group_embeddings = []
            for image_name in group_image_names:
                group_embeddings.append(dino_embeddings_per_image[image_name])
            # Calculate the average embedding for this appearance group
            avg_embedding = torch.mean(torch.stack(group_embeddings), dim=0)
            dino_embeddings_per_appearance[appearance_id] = avg_embedding
        ...
        if self.params.points_from == "sfm":
            ...
            xyz, rgb, _ = ColmapDinoDataParser.read_points3D_binary(
        ...
        return DataParserOutputs(
            ...
            dino_embedding_size=dino_embedding_size,  # Add dino_embedding_size
            dino_embeddings_per_appearance=dino_embeddings_per_appearance, # add dino per group embeddings
        )
